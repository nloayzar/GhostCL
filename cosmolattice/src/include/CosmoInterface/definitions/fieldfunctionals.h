...
namespace TempLat {
    ...
    public:
        ...
        // --> Ghost scalars
        template<class Model, int I>  // <Grad[f]^2>
        static inline auto grad2GS(Model& model, Tag<I> i)
        {
            return Grad2<Model::NDim>(model.fldGS(i));
        }

        template<class Model, int I>  // <pi^2>
        static inline auto pi2GS(Model& model, Tag<I> i)
        {
            return pow<2>(model.piGS(i));
        }
