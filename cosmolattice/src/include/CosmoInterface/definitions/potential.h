...
namespace TempLat {
    ...
    public:
        ...
        template<class Model>
        static inline auto ghostpotential(Model& pM)
        {
            return Total(i,0,Model::NGhostPotTerms-1,pM.ghostpotentialTerms(i));
        }


        template<class Model>
        static inline auto ghostmass(Model& pM)
        {
            return Total(i,0,Model::NGhostMassTerms-1,pM.ghostMassTerms(i));
        }
        ...
        // First derivative of the potential wrt scalar field n.
        template<class Model, int N>
        static inline auto derivGS(Model& model, Tag<N> n)
        {
            return model.potDerivGS(n); // calls function in model file
        }

        // First derivative of mass terms of the potential wrt scalar field n.
        template<class Model, int N>
        static inline auto derivMassGS(Model& model, Tag<N> n)
        {
            return model.potMassDerivGS(n); // calls function in model file
        }
        ...
        // Second derivative of the potential wrt scalar field n. 
        template<class Model, int N>
        static inline auto deriv2GS(Model& model, Tag<N> n) 
        {
            return model.potDerivG2(n);
        }

        // Second derivative of the potential wrt scalar field n. 
        template<class Model, int N>
        static inline auto Mass2S(Model& model, Tag<N> n) 
        {
            return model.potDerivMass2(n);
        }
