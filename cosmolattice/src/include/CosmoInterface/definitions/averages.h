...
namespace TempLat {
    ...
    public:
        ...
        // --> Ghost Scalar singlet:

        template<class Model>  
        static inline auto grad2GS(Model& model)  // <Grad[f]^2> (sum over fields)
        {
            return average(Total(i,0,Model::NGs - 1, FieldFunctionals::grad2GS(model,i)));
        }

        template<class Model> 
        static inline auto pi2GS(Model& model)  // <pi^2 [f]> (sum over fields)
        {
            return average(Total(i,0,Model::NGs - 1,FieldFunctionals::pi2GS(model,i)));
        }
        ...
        static inline void setAllAverages(Model& model) {
            model.potAvI = average(Potential::potential(model)) + average(Potential::ghostpotential(model)) - average(Potential::ghostmass(model));
            model.potAvSI = average(Potential::potential(model)) + average(Potential::ghostpotential(model)) - average(Potential::ghostmass(model));
            ...
            if (model.NGs > 0) {
                model.GSpi2AvI = Averages::pi2GS(model);
                model.GSpi2AvSI = model.GSpi2AvI;
                model.GSgrad2AvI = Averages::grad2GS(model);
            }
            if (Model::NU1 > 0) {
