...
namespace TempLat {
    ...
    public:
        ...
        void evolve(Model& model, T tMinust0) {
            ...
            if (model.NGs > 0) kickGhostScalar(model, weight);
            ...
            if (model.NGs > 0) driftGhostScalar(model);
        ...
        void sync(Model& model, T tMinust0) {
            if(! synced) {
                ...
                if (model.NGs > 0) kickGhostScalar(model, 0.5);
                ...
                if (expansion) {
                    ...
                    if (Model::NGs > 0) model.GSpi2AvI = Averages::pi2GS(model); // at t
        ...
        *********/
        ...
        template<class Model>
        void kickGhostScalar(Model& model, T w) {
            ForLoop(n, 0, Model::NGs -1,
                    model.piGS(n) += (w * model.dt) * GhostScalarSingletKernels::get(model,n);
            );
        }
        ...
        *********/
        ...
        template<class Model>
        void driftGhostScalar(Model& model) {
            // Here we rescale the canonical momentum appropriately to get
            // the time derivative of the field.
            model.fldGS +=  pow(model.aSI, model.alpha - 3) * (model.dt * model.piGS );
        ...
        *********/
        ...
        void storeMomentaAverages(Model& model) {
            if (Model::Ns > 0) {
             ...
             if (Model::NGs > 0) {
                model.GSpi2AvSIM = model.GSpi2AvSI;  // at t-dt/2
                model.GSpi2AvSI = Averages::pi2GS(model); // at t+dt/2
                model.GSpi2AvI = 0.5 * (model.GSpi2AvSIM + model.GSpi2AvSI); // at t (average)
            }
        ...
        void storeFieldsAverages(Model& model) {
            ...
            if (Model::NGs > 0) model.GSgrad2AvI = Averages::grad2GS(model); // at t
            ...
            model.potAvI = average(Potential::potential(model)) + average(Potential::ghostpotential(model)) - average(Potential::ghostmass(model)); // at t
        }
