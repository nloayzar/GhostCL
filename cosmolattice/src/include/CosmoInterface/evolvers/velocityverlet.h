    ...
    public:
        ...
        void evolve(Model &model, T tMinust0) {
            ...
            for(size_t i = 0; i < stages; ++i ){  // loop over operations...
                ...
                if(model.NGs > 0) kickGhostScalar(model, w);
                ...
                if(model.NGs > 0) driftGhostScalar(model, w);
        ...
        *********/
        ...
        void kickGhostScalar(Model &model, T w) {
           ...
           ForLoop(i, 0, Model::NGs - 1,
           		model.piGS(i) += (w * model.dt / 2) * GhostScalarSingletKernels::get(model, i) ;
           			);
        }
        ...
        *********/
        ...
        // Evolves fldGS
        template<class Model>
        void driftGhostScalar(Model &model, T w)
        {
            model.fldGS +=  pow(model.aSI, model.alpha - 3) * (model.dt * w * model.piGS );
        }
        ...
        *********/
        ...
        void storeMomentaAverages(Model& model) {
            ...
            if (Model::NGs > 0) model.GSpi2AvI = Averages::pi2GS(model);
        ...
        void storeFieldsAverages(Model& model) {
            ...
            if (Model::NGs > 0) model.GSgrad2AvI = Averages::grad2GS(model);
