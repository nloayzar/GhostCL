...
namespace TempLat {
    ...
    public:
        ...
        static auto get(Model& model)
        {
            ...
            if(model.NGs > 0){ 
                Ekgs = Energies::kineticGS(model);  //  scalar singlet, kinetic energy
                Egghs = Energies::gradientGS(model);  //  scalar singlet, gradient energy
            }
            if(model.NCs > 0){
            ...
            return pow(model.aI, 2 * model.alpha + 1) / 3.0 * pow<2>(model.fStar / Model::MPl) *
                   ((model.alpha - 2) * (Eks - Ekgs + Ekcs + EkSU2Dbl) + model.alpha * (Egs - Egghs + Egcs + EgSU2Dbl) +
        ...
        static auto getSI(Model& model)
        {
            ...
            if(model.NGs > 0){ 
                Ekgs = Energies::kineticSSI(model);  //  scalar singlet, kinetic energy
                Egghs = Energies::gradientSSI(model);  //  scalar singlet, gradient energy
            }
            if(model.NCs > 0){
